...
/* Made for Tic-Tac-Toe integration */
double moveProbability[2] = {0}; // made for Tic Tac Toe
double featuresCount[DEPTH][ROW][COL] = {0}; // count for each square and each configuration (x,o and blank)
double winCount[2] = {0}; // count for each win / loss
int infoCount = MAX; // total number of rows
...
int main()
{
    ...
    split();
    learn();
    printf("Calculating time taken by AI to compute it's moves:\n");
    ...
    printf("it took %f seconds",time_spent);
...
// Naive Bayes
// In integration, uses the whole dataset!
void learn() { // function for learning from the dataset.
    ...
    for (int i = 0; i < MAX; i++) {
        if (strcmp(result[i], "positive") == 0) { // if the result is 'positive' of the current row
            winCount[0]++; // add 1 to the postive count to the list
            toIncrement = 0; // set it so that for all the choices for each square of the current row will be added with a positive label
        }
        if (strcmp(result[i], "negative") == 0) { // if the result is 'negative' of the current row
            winCount[1]++; // add 1 to the negative count to the list
        ...
        for (int k = 0; k < DEPTH; k++) { // loop through all 9 slots of the current row
             switch (extractedData[i][k]) {
                case 'b': // if blank add
                featuresCount[k][0][toIncrement] ++; 
                break;
                case 'x': // if x add
                featuresCount[k][1][toIncrement] ++;
                break;
                case 'o': // if o add
                featuresCount[k][2][toIncrement] ++;
                break;
             }
        }   
    }
    ...
    for (int i = 0; i < DEPTH; i++) { // loop through each square
        for (int k = 0; k < ROW; k ++) { // loop through each state blank, 'x', 'o'
            for (int p = 0; p < COL; p ++) { // loop through the 'positive' and 'negative'
                featureProbability[i][k][p] = featuresCount[i][k][p] / winCount[p]; // take the no of 'positives' of each square and each state, divide it by the overall 'positives' of the dataset... same for 'negative'
            }
        }
    }
    // calculate outcome probability
    for (int i = 0; i < COL; i++) {
        winProbability[i] = winCount[i] / MAX; // probability of it being a positive or negative in relation to the dataset.
    }
}
...
// created for tic-tac-toe integration! adds new data... might not affect game!
void relearn() {
    infoCount ++;
    int toIncrement = 0; // sets an int value for indexing later for 'positive' and 'negative' (0 for positive, 1 for negative)
    if (winner == 1){
        winCount[0]++;
        winProbability[0] = winCount[0] / infoCount;
        toIncrement = 0;
    } else {
        winCount[1]++;
        winProbability[1] = winCount[1] / infoCount;
        toIncrement = 1;
    }
    for (int i = 0; i < ROW; i ++) { // begin loop to look through board
        for (int k = 0; k < ROW; k ++) {
